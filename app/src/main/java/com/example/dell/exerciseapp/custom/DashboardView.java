...
import com.example.dell.exerciseapp.entity.HighlightCR;
    ...
    private String[] getMeasureNumbers() {
        ...
        for (int i = 0; i <= mBigSliceCount; i++) {
            if (i == 0) {
                ...
                int divide = Math.abs(mMinValue) / mBigSliceCount;
                if (Math.abs(mMinValue) % mBigSliceCount != 0 ){
                     strings[i] =String.valueOf(mMinValue + mBigSliceCount * i);
                     if (i> divide +1){
                         strings[i] = String.valueOf(((mMaxValue- Integer.parseInt(strings[divide+1]) ) / mBigSliceCount ) * i);
                     }
                }
